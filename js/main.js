...
let googleArr = arr.map((tactObject) => {
...
function draw1() {
    ...
    let datad3 = arr;
    let margin = {top: 10, right: 100, bottom: 30, left: 40};
    ...
    let xScale = d3.scaleLinear().domain([0, Init.NUM_TACT]).range([0, width]);
...
function draw2() {
    ...
    let xScale = d3.scaleLinear().domain(d3.extent(datad3, d => d[0])).range([0, width]);
    let yScale = d3.scaleLinear().domain(d3.extent(datad3, d => d[1])).range([height, 0]);
...
function draw3() {
    ...
    let svg2 = d3.select('#chart3').select('svg')
        .append('g')
        .attr('transform', `translate(${margin.left}, ${margin.top})`);
    ...
    function render(data) {
        ...
        let updateText2 = svg2.selectAll('text')
            .data(data, (d) => d);
